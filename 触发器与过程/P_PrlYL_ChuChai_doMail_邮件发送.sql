create or replace procedure P_PrlYL_ChuChai_doMail(p_FlowGid varchar --流程Gid
                                                   ) as
...
begin
  ...
  for R in (select f.*, wm.name ModelName
    ...
    v_Content := v_Content || v_TStart || v_Body || v_TEnd;
    v_Body    := '[出差时间] : ' || to_char(R.CCBDate, 'YYYY.MM.DD') || '~' ||
                 to_char(R.CCEDate, 'YYYY.MM.DD');
    v_Content := v_Content || v_TStart || v_Body || v_TEnd;
    v_Body    := '[出差地点] : ' || R.CCPlace;
    v_Content := v_Content || v_TStart || v_Body || v_TEnd;
